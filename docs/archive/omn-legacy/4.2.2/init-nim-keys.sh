...
# ─────────────────────────────────────────────────────────────
# NIM OmniRoute initializer  v4.2.2（基于 v4.2.1）
# 相对 v4.2.1 的变更：
#   【v4.2.2·⑦ 】combo 创建改为幂等 upsert_combo（存在则 PUT，不存在才 POST）
#              根治 R2 restore 后 first-init 裸 POST 撞 "Combo name already exists"。
#   【v4.2.2·⑧ 】增量门放宽：任一 nim-* combo 存在或 INIT_MARKER 存在即走增量，
#              不再仅凭 nim-pool，避免半成品状态反复走 first-init。
#   继承 v4.2.1：① 移除 quota-share/主池 p2c+白名单 ② nim-codex 响应体打印
#              ③ DEBUG log 不入 Dataset ④ 可选探针 NIM_PROBE
#              ⑤ 增量只清过期熔断 ⑥ nim_health_pick 分档推荐。
# ─────────────────────────────────────────────────────────────
