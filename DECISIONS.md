...
- **dev/prod 隔离核** (push 前必核, 已核毕): sync-space-nonoke.yml `push: branches: [main] paths: [Dockerfile]` 自动触 dev; **sync-space-nomke.yml 仅 `workflow_dispatch` (圣上显令点火, 无 push 触发)**。push Dockerfile 改 ARG 到 main → 仅 dev nonoke/omn 自动切 3.8.48, prod nomke/omn 仍 9c9aecfd 直到圣上 workflow_dispatch 显令。code-level 隔离钉死非靠人守, 24h 窗口有效。
