...
- ⏳ **FT Worker 拓扑全可配变量化已落码待启用** (2026-08-23 落码, deploy-ft-workers.yml + DECISIONS §10): 新增 `WORKERS_PER_ACCOUNT` 变量 (默认 3, 甜点) + 账号数 `ACTIVE_ACCOUNTS` (**全账号都用上**: Variable 已设 10=f01~f10 全 zone), **两端全可配**贯穿全链 (矩阵/gen-names/deploy POS/publish worker-major 派生). 新增 `PRESET=reorg` 拓扑重组专用 (先重生成名→删旧 worker→全量建新). 查证结论: 单 CF 账号 2-3 worker 收益最大 (共享 Anycast 出口池 + 共享 100K 配额), 扩 IP 多样性的杠杆是加账号数非加每账号 worker 数. 待圣上触发 `PRESET=reorg` 完成拓扑收缩到 **10×3 (全账号×每账号3)** + 推 HF Dataset 真启用
