### **结论：以下是从 output.md 全文搜索查证后提炼的所有部署脚本优点，按功能类别逐条详细列出，已跨版本去重合并。**

---

### 一、基础设施与编排

**镜像 Tag+Digest 双写锁定**（v4.2.3）——`diegosouzapw/omniroute:3.8.43@sha256:517c160643c56ad72e3e305458d961c9a4c87f711393c13020450f9f088d1570`，digest 保证不可变，tag 便于人读，禁止浮动 `latest` 导致 Turbopack 构建漂移

**跨版本防御环境变量**（v4.2.3）——`OMNIROUTE_USE_TURBOPACK=0` 强制走 webpack 绕开 3.8.45+ 的 Docker Turbopack 缓存 mmap 失败；`OMNIROUTE_MAX_PENDING_MIGRATIONS=0` 防止旧库补 migration 时 abort 刷屏中断

**Litestream v0.5.9 集成**（v4.3.0）——修复 R2 `InvalidContentEncoding`，`auto-recover: false` 防止 replicate 启动时自恢复绕过 entrypoint guard 覆盖有效 DB

**entrypoint 优雅降级 restore**（v4.3.1）——`litestream restore ... && echo complete || echo WARN: Continuing`，restore 失败不阻断，空库继续启动

**entrypoint 原子 restore**（v4.3 candidate）——临时路径 restore + `PRAGMA quick_check` 验证 + 原子 mv，防止 restore 中断损坏正式 DB

**entrypoint PID 1 进程监督循环**（in4.3）——保持 PID 1，显式监督 OmniRoute、Gate、Litestream 三进程；任一核心进程退出则容器退出交由编排层重启；Litestream 退出可配置为告警不拖垮容器

**entrypoint trap 信号转发**（v4.3 candidate）——`trap shutdown INT TERM EXIT`，向所有子进程发 SIGTERM，3 秒 grace 后 SIGKILL，wait 回收，无孤儿进程

**entrypoint 健康等待绝对时间戳截止**（in4.3）——用 `deadline=$(( $(date +%s) + 180 ))` 替代计数器循环，避免调度暂停导致计数失准

**entrypoint 版本护栏**（v4.3.1）——`EXPECTED_OR_VERSION` 检查，版本不一致只告警不中断；`STRICT_VERSION_LOCK=1` 时直接退出阻止静默漂移
...
**entrypoint `|| INIT_EXIT=$?` 捕获**（v7）——捕获非零退出码而不触发 `set -e`，避免非致命错误导致容器崩溃

**Dockerfile HEALTHCHECK 对齐**（v4.2.3）——`start-period=180s` 与 entrypoint 内部 180s 等待对齐，`retries=3` 容忍瞬时抖动

**Dockerfile 精简依赖**（v4.2.3）——仅安装 curl/jq/python3/python3-pip/sqlite3/ca-certificates + huggingface_hub，`--no-install-recommends` 减小镜像体积
...
**gate.js PSK 最小长度 16 fail-closed**（v4.3 candidate）——缺失或过短即 FATAL exit，防止弱 PSK 配置
...
**gate.js 共享预算限流**（in4.3）——Gate 自行执行内存令牌桶：RPM 滑动窗口 + 并发计数 + 间隔 pacing 三层整形，不依赖 OmniRoute 内部 Request Queue API

**gate.js 限流响应含 Retry-After**——并发超限返回 `429 + Retry-After: 2`；RPM 超限计算剩余秒数；间隔未到计算等待秒数

**gate.js `/v1/search/analytics` 单独 404**——防止 PSK 持有者看到搜索分析数据

**gate.js 诊断端点**（v5）——`/gate/diagnostics` 返回错误统计（502/503/504 计数）、最近 20 条请求日志和配置快照

**gate.js 结构化请求日志**（v5）——每个请求记录为 JSON 对象（含 ts/method/path/statusCode/latencyMs/contentLength/userAgent）
...
**Cookie login 请求体用 jq 构造**（v4.3.1）——`jq -n --arg password "$INITIAL_PASSWORD" '{password:$password}'` 替代内联 JSON 字符串
...
**保守整形策略**（v4.3/in4.3）——默认固定 28 RPM / 1 并发 / 2200ms，通过"整形"而非"扩容"显著降低 429 熔断率
...
**Resilience 读回逐字段核对**（v4.3.1）——PATCH 后 GET 读回 RPM / minMs / concurrent，逐字段与预期比对，不一致 `exit 1`
...
**删除 Resilience 猜测字段**（in4.3）——彻底删除 `providerBreaker` / `connectionCooldown` 猜测字段；官方文档证实熔断仅对 408/500/502/503/504 触发不含 429

**jq 引号 BUG 修复**（v4.3.1）——变量预取替代 `'"'"'` 堆叠，消除 jq 引号转义错误

**Gate 内存令牌桶不依赖 OmniRoute API**（in4.3）——Gate 自行强制执行共享预算，不依赖 OmniRoute 内部 Request Queue API
...
**幂等 upsert_combo**（v4.2.3）——存在则 PUT，不存在才 POST，可安全重复执行
...
**purge_proxy_db 收窄清理范围**（v5）——仅清理脚本管理的 relay，保留用户手动添加的
...
**apply_context_override 恢复**（K5 修复）——从 42ea8e7 基线恢复 `source='init'` override，弃用不存在的 API PATCH 路径
...
**Context Override 改用官方 API**（4.3.txt）——通过 `/api/provider-models` PATCH 而非直接 SQL 写表，避免 migration 结构漂移
...
**OmniRoute 原生 `/v1/search` 统一入口**——内置 5-9 家 Provider（Serper/Brave/Perplexity/Exa/Tavily/SearXNG 等），自带 auto-failover 和 cache
...
**HF Dataset 快照（HF Hub API commit）**（v5）——原子操作 + base64 编码，无需 git 依赖，提供配置版本历史
...
**`_hf_snapshot || true` 防护**（v7）——快照失败不阻塞部署流程
...
**单变量调试 NIM_MODE=DEBUG**（v4.1.0）——单一变量联动三项行为：日志 tee 落盘 + APP_LOG_TO_FILE + DISABLE_SQLITE_AUTO_BACKUP 防写锁冲突

**DEBUG 日志保留最近 N 份**——`NIM_DEBUG_LOG_KEEP` 控制保留数量，自动清理旧日志

**密码通过环境变量直接传递**（v7）——由 OmniRoute 自身处理明文到 bcrypt 迁移，消除 SQL 引号错误

**SQL 注入防护 `sql_escape()`**（v5）——所有用户可控变量嵌入 SQL 前经过 `sed "s/'/''/g"` 转义

**RELAY_AUTH_KEY 外部化**（v5）——从硬编码改为环境变量读取，消除凭证泄露风险

**项目结构扁平化**（v7）——6 个核心文件全部位于根目录，无子目录嵌套

**sync-to-hf.yml 健壮性增强**（v7）——`git rm --ignore-unmatch || true` + `DEPLOY_COMMIT` 改用 `${{ github.sha }}`

**README.md HF 元数据合规**（v7）——移除字符串格式 `datasets` 字段（HF 要求数组），消除 pre-receive 验证失败

**`.env.example` 完整文档**（v5）——56 个环境变量 + 中文注释，按功能分区，是接手者的配置参考手册

**b64() 函数 BusyBox 回退**（v5）——优先 GNU coreutils `-w 0`，回退 `tr -d` 方式，确保跨发行版兼容
...
**SQLite 查询式初始化判断**（v4.5.1）——摒弃文件标记方案，直接查询数据库判断初始化状态，解决容器重建后临时文件消失问题
