...
## §10 FT Worker 拓扑全可配变量化 (2026-08-23 圣上令, deploy-ft-workers.yml, 只增不改)

圣上 2026-08-23 先令"搜索论证单 CF 免费账号建几个 FT worker 收益最大"后批"账号数和worker数全都为可配变量"。
...
**新增 `PRESET=reorg` 拓扑重组专用选项** (圣上令"执行时加删旧 worker 选项"): 一次触发完成拓扑收缩 + 孤儿清理 + 全量重建。
