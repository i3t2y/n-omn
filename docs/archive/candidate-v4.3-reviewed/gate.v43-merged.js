...
//       后台关时即使持上游 Cookie 也 404 — 不泄露后台是否存在, 堵 /api/auth/login 撞 INITIAL_PASSWORD 面。
...
//       通过后删 Authorization 头不转发上游 (防凭据泄露); 上游自家 Cookie/Session 照走。
...
// R3+ Restart A (i′ 方案 A): CONCURRENT_LIMIT 1→3 (第一杠杆 R5; gate CONCURRENT_LIMIT=1 下
// slow/tail reasoning 模型占槽致 429 组1v2 死锁根因). env NIM_CONCURRENT_LIMIT 覆盖默认.
const CONCURRENT_LIMIT = parseInt(process.env.NIM_CONCURRENT_LIMIT || '3', 10);
...
http.createServer((req, res) => {
  ...
  //   后台关时即使持上游 Cookie 也 404 (不泄露后台是否存在)。
  ...
  if (adminMode) {
    // 后台: 删 Basic Auth 凭据不转发上游 (防泄露); 上游自家 Cookie/Session 照走
