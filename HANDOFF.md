...
- **429 风暴诊断 (2026-08-19 红外, 详 ops/STATUS + DECISIONS §5)**:
  ...
  - **2026-08-20 12:05 boot 实证更新 (三病根)** — 病根①**已解除** (凭据齐 has_r2=1, boot 走 restore 分支非 skip), 病根②**已解除** (圣上处置 PERSIST, boot 印 `Litestream PID=151` 开态非关态), **病根③实锤真根** = **litestream.yml `path` 硬编码 `/app/data/storage.sqlite` 对不上运行 DB path**: entrypoint 默认 DATA_DIR=/app/data (L16) 但 HF Space env 设 `DATA_DIR=/data` → 运行 DB 实际 `/data/storage.sqlite` (boot 印 `SQLite database ready: /data/storage.sqlite`), restore 报 `database not found in config: /data/storage.sqlite` → **`⚠ restore 失败 rc=1 空库启动`** = R2 持久化真障碍。**修法 (commit c790e05)**: litestream.yml path 改 `$\{DATA_DIR\}/storage.sqlite` env 派生对齐运行态 (entrypoint L17 export DATA_DIR 保证子进程见 /data; litestream.yml 其它键 bucket/endpoint/keys 全用 `${ENV}` 展开且已工作实证 replicate 能写 R2, path 同受支持)。**部署**: dev/logic → push nomn → sync-logic-nonoke → Dataset → Restart → 复验 v1-v5 (restore 应成 config 找得到 DB)。若复验仍 `database not found in config` = path 修未生效 (sync 未上 Dataset / env 展开不支) 回查。**验签判定**: 首 boot 复验 `restore rc=0 原子 mv` 或 `无副本空库启动` + replicate PID + init rc=0 + litestream sync 写 R2 + R2 Dashboard 首个 generation; 二 boot 真持久化 (restore 拉真库 + manage key 跨 boot 存)。
