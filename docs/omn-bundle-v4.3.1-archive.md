# omn-merge · v4.3.1 存档版合并稿（docs/archive/4.3.1.txt）

> 来源: `docs/archive/4.3.1.txt`（CRLF 行尾, Jul 14 20:43 存档快照）.
> 修正: README 节 4.3.1.txt 原文 `title: Omn`（编码截断损坏）→ 用根目录 README.md frontmatter 补全 (title: OmniRoute Candidate v4.3).
> 其余 6 脚本: 去 CRLF（^M）保留原内容, 与根部署版实质差异见对比说明.
> 生成日期: 2026-07-14
...
| 文件 | 状态 | 说明 |
|------|------|------|
| litestream.yml | 等价 | 内容一致 |
| package.json | 等价 | version=4.3.0 一致 |
| Dockerfile | 注释+结构 | 此版 gate.js 用纯 http/fs, Dockerfile 声称 '无需 npm install' (根版用 express + npm install) |
| gate.js | 架构级不同 | 纯 http/fs 无 express (根版用 express 框架) |
| entrypoint.sh | 大幅精简 | 此版无完整 trap/shutdown/LITESTREAM_STRICT 逻辑 (根版含完整时序) |
| init-nim-keys.sh | 大幅精简 | 此版去 v4.2.2 继承注释链, 精简 (根版 v4.3.0 含完整注释) |
| README.md | 原损坏已修 | 4.3.1.txt 原文 title 截断为 `Omn`, 已用根 frontmatter 修正 |

## 文件清单

`Dockerfile` · `README.md` · `entrypoint.sh` · `gate.js` · `init-nim-keys.sh` · `litestream.yml` · `package.json`
...
# gate.js 仅用 Node 内置模块（http/fs），无需 npm install
...
if (!OR_API_KEY) {
  try { OR_API_KEY = require('fs').readFileSync('/data/.or-api-key', 'utf8').trim(); }
...
const server = http.createServer((req, res) => {
  ...
  // 透明反向代理到本地 OmniRoute
  const headers = { ...req.headers, host: `127.0.0.1:${OR_PORT}` };
  const proxyReq = http.request(
    { host: '127.0.0.1', port: OR_PORT, path: req.url, method: req.method, headers },
    (up) => {
      res.writeHead(up.statusCode || 502, up.headers);
      up.pipe(res); // 流式转发，支持 SSE
    }
  );
...
// SSE / 长连接：禁用超时，避免大上下文压缩期间被网关掐断
server.timeout = 0;
server.requestTimeout = 0;
server.headersTimeout = 0;
server.listen(GATE_PORT, '0.0.0.0', () => {
  console.log(`[gate] listening on 0.0.0.0:${GATE_PORT} -> 127.0.0.1:${OR_PORT} (zero-dep)`);
});
...
[init-nim-keys.sh — bash]

```bash
#!/bin/bash
set -eo pipefail
# ─────────────────────────────────────────────────────────────
# NIM OmniRoute initializer  v4.3.1（基于跑通的 v4.3.0）
# 相对 v4.3.0 的变更（仅 4 处，其余全量不动）：
#   【FIX-1 根因】清 settings 全局代理 + SQL 兜底：
#        ECONNREFUSED 127.0.0.1:20129 来自 OmniRoute 启动时的
#        [STARTUP] Global fetch proxy patch（内存全局 dispatcher），
#        其配置源是 settings 表的 proxyUrl，非环境变量、非 proxy_registry。
#        undici fetch 设计上不读 HTTP_PROXY，故 unset/check_dangerous_env 无效。
#        Settings PATCH 补 proxyUrl=null/proxyEnabled=false/relayBackend=null，
#        触发 [HOT_RELOAD] 热重载重建 dispatcher；purge_proxy_db 加 SQL 兜底。
#        ⚠️ 若热重载无法重建 dispatcher，兜底须重启 OmniRoute 进程（见 entrypoint）。
#   【FIX-2】Resilience 日志 jq 引号 BUG 修复（'"'"' 堆叠 → 变量预取）。
#   【FIX-3】nim_probe：--retry 2 + HTTP 000（传输失败）不计入 bad，
#        防 compaction 磁盘 I/O 抖动误杀模型。
#   继承 v4.3.0 全量：动态限流、context_accumulator、nim_probe、
#        nim_health_pick、Thinking/Memory API、Resilience 白名单+读回等。
# ─────────────────────────────────────────────────────────────
# ══ 单变量调试 + 日志归档 ═══════════════════════════════════════
...
# ── 强制关闭代理生态（注：undici fetch 不读这些 env，仅防御 http/https 模块）──
...
# models_to_json: 每个模型名 printf 单独成行再加 nvidia/ 前缀转 JSON，combo 模型列表才正确
models_to_json() { printf '%s
' "$@" | sed 's/^/nvidia\//' | jq -R '{model: .}' | jq -s -c .; }
# ══ combo 策略白名单（删 context-relay：CF-1 红线）═══════════
...
# ══ 动态限流（按 alive_keys 线性扩，上限 300 RPM）══════════
...
purge_proxy_db() {
  ...
  if [ -f "$_DB_PATH" ]; then
    ...
    # ── 【FIX-1】清 settings 表全局 proxy/relay（内存 dispatcher 的真正配置源）──
    # ⚠️ 表名/字段名需按 3.8.43 实际 schema 核对（先 .schema settings 确认）。
    #    以下按 key='runtime' + JSON value 的常见形态兜底，失败静默不阻断。
...
' "$NIM_KEYS" | sed '/^[[:space:]]*$/d')
  ...
  while IFS= read -r m; do
    ...
    # 【FIX-3】--retry 2 抗抖动
    _code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 20 --retry 2 --retry-delay 1 \
    ...
    echo "$_now" > "$_stamp"
    # 【FIX-3】HTTP 000（传输失败/超时）不判死，避免 compaction I/O 抖动误杀
    if [ "$_code" != "200" ] && [ "$_code" != "000" ]; then
      echo "$m" >> /tmp/nim-probe-bad.txt
    fi
    sleep 1
  done < <(build_all_models)
}
...
# ══ context_accumulator（含 tokens_in>0 过滤）══════════════
...
purge_proxy_db
# ── Resilience PATCH 白名单 + 输入校验 + 读回断言 ──
...
# 【FIX-2】jq 引号 BUG 修复：变量预取，不再堆叠 '"'"'
_sent_keys=$(echo "$RESILIENCE_BODY" | jq -rc 'keys | join(",")' 2>/dev/null || echo "parse_err")
echo "[init] Resilience PATCH body keys=[$_sent_keys] (无顶层 useUpstream429BreakerHints)"
...
# ── 【FIX-1】Routing PATCH：补 proxyUrl=null/proxyEnabled=false/relayBackend=null ──
# 触发 [HOT_RELOAD] 热重载重建内存全局 fetch dispatcher，消除 20129 ECONNREFUSED。
echo "[init] Routing + maxBodySizeMb=$_REQUEST_BODY_LIMIT_MB (v4.3.1: 清 settings 代理，重置内存 dispatcher)..."
SETTINGS_CODE=$(curl -s -o "$SETTINGS_RESP_FILE" -w "%{http_code}" -b "$COOKIE_FILE" \
  ...
  -d "{\"fallbackStrategy\":\"$_FALLBACK_STRATEGY\",\"stickyRoundRobinLimit\":$_STICKY_LIMIT,\"requestRetry\":2,\"maxRetryIntervalSec\":5,\"maxBodySizeMb\":$_REQUEST_BODY_LIMIT_MB,\"proxyUrl\":null,\"proxyEnabled\":false,\"relayBackend\":null}")
...
# 读回确认 settings 代理已清（诊断用，不阻断）
_SB=$(curl -s -b "$COOKIE_FILE" "$BASE_URL/api/settings" 2>/dev/null)
_SB_PROXY=$(echo "$_SB" | jq -r '.proxyUrl // "null"' 2>/dev/null || echo "parse_err")
echo "[init] settings 读回: proxyUrl=$_SB_PROXY（期望 null）"
[ "$_SB_PROXY" != "null" ] && echo "[init] ⚠️ WARN: settings.proxyUrl 未清空，内存 dispatcher 可能仍指向旧代理；若请求持续 ECONNREFUSED 20129 需重启 OmniRoute 进程。"
...
hf_snapshot() {
  ...
  # DEBUG log 上传默认关闭（NIM_DEBUG_LOG_TO_DATASET=1 开启）
