...
#   v3.7.0  【本次·查证修正 + 变量对齐】
#     【查证A】context-relay 仅用于“账号轮换 handoff 摘要”，且当前只支持 Codex 轮换，
#              对 nvidia/NIM 空转 —— 不吃 CPU。之前“改 round-robin 省 CPU”的说法作废。
#              真正防超长文 502 的是：CONTEXT_LENGTH_DEFAULT(32K override) + 压缩 + body limit。
#     【修复D·关键】脚本变量名与 HF 面板不一致导致调参不生效，现对齐：
#              NIM_REAL_CONTEXT      -> CONTEXT_LENGTH_DEFAULT
#              NIM_REQUEST_BODY_LIMIT_MB -> NIM_REQUEST_BODY_LIMIT
#     【修复E】nim-codex 由 context-relay 改 round-robin（对 NIM 语义更诚实，非性能因素）。
#              如你确实要给 Codex 用，把 _CODEX_STRATEGY 改回 context-relay 即可。
#     【保留】RPM 默认 28（尊重用户设置，16G 内存充足，不再下调）。
# ─────────────────────────────────────────────────────────────
...
_STICKY_LIMIT=1
_REQUEST_BODY_LIMIT_MB=${NIM_REQUEST_BODY_LIMIT:-1}          # ★修复D：去掉 _MB 后缀
...
echo "[init] Starting NIM OmniRoute initializer v3.7.0..."
...
apply_context_override() {
  if sqlite3 "$_DB_PATH" \
    ...
    OVERRIDE_APPLIED=$((OVERRIDE_APPLIED+1)); else OVERRIDE_SKIPPED=$((OVERRIDE_SKIPPED+1)); fi
}
...
if [ -f "$_DB_PATH" ]; then
  ...
  if [ "${COMBO_COUNT:-0}" -gt 0 ]; then
    ...
    hf_snapshot
    echo "[init] Done (incremental). v3.7.0"
    exit 0
  fi
else
...
echo "[init] Done (first-init). v3.7.0"
