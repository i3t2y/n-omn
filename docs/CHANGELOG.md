...
- **[优化] 双 PRAGMA 合一**（`#4`）：旧版 `_detect_input_col`/`_detect_output_col` 各跑一次 `PRAGMA table_info(call_logs)`，对同一表重复扫两遍。合一为 `_detect_io_cols` 单次 PRAGMA + awk 双模式各取列名，省一次磁盘扫。输出改两行（input/output 列名各一行）而非 `\t` 分隔——避免 IFS tab 空白折叠致列错位（Bug A 同类回归：仅 output 命中时前导 tab 被 read 剥离，output 列名错位落入 input 字段）。
- **[优化] `_arr_json` JSON 构造改 jq 正规转义**（`#7`）：旧版用 bash 字符串拼接 `\"$_i\"`构造 JSON 字符串数组，未转义双引号/反斜杠，比既有 `models_to_json`（line 102 用 `jq -R`）更脆弱——模型名含 `"` 或 `\` 即坏 JSON。改 `printf '%s\n' "$@" | jq -R . | jq -s -c .` 正规转义，空数组兜 `[]`。当前 NIM slug 无特殊字符非触发，是为加固防未来。
- **[修复] context_accumulator_update 过滤 model-sync 噪声**：累积判读 SELECT 加 `AND model LIKE '%/%' AND model != 'model-sync'`。`model-sync` 等内部进程记录（无 `/` 的非模型 ID）不再扫进 `context_recommendations` 表，排除噪声条目污染置信度。严格版 `LIKE '%/%'` 只纳含 `/` 的真实模型 ID（如 `z-ai/glm-5.2`）。
### **v4.2.2 (2026-07-09)**
