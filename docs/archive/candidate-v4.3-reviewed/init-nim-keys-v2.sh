#!/bin/bash
# NIM 初始化 · 完整自包含版 (K3 v2.0)
# API 路径全部对照上游 Wiki 修正 (https://github.com/diegosouzapw/OmniRoute/wiki/API-Reference)   # 注: 上游仓库名保留不改 (URL 路由功能性)
...
log() { echo "[init] $*"; }
# 管道尾部回调改用 { read -r _v; ... && log ...; } (内联子shell 共享函数定义),
...
# 读回已是 {global:null, providers:{}, combos:{}, keys:{}} 即关态 → 跳过 PUT (避免 400 + schema 猜测).
# SQL 兜底在 Step 6 已清代理表, 这里仅读回确认 + 仅在 global 非 null 时强写回关态.
...
if [ -n "$_g" ] && [ "$_g" != "null" ]; then
  curl -s -o /dev/null -w "[init]   proxy disable PUT: HTTP %{http_code}\n" -b "$COOKIE" \
    -X PUT "$BASE/api/settings/proxy" -H "Content-Type: application/json" \
    -d '{"global":null}' 2>/dev/null || log "  ⚠ proxy PUT 失败 (非阻断, Step 6 SQL 兜底)"
else
...
curl -sf -b "$COOKIE" "$BASE/api/settings/compression" 2>/dev/null \
  | head -c 300 | { read -r _v; [ -n "${_v:-}" ] && log "  compression 当前: $_v"; }
...
curl -sf -b "$COOKIE" "$BASE/api/settings/thinking-budget" 2>/dev/null \
  | head -c 300 | { read -r _v; [ -n "${_v:-}" ] && log "  thinking-budget 当前: $_v"; }
