...
### 7.7 `stream_options` 坑 ⭐

NIM API 比 OpenAI API 更严格：`stream_options` 必须与 `stream: true` 共存。

OpenAI SDK（Hermes Agent 等）会自动添加 `stream_options` 但不一定设 `stream: true`，导致 400 错误。

gate.js PATCH-GATE-003 已处理：
...
- 无文件时声称已经基于行号精确修改。
- 重新生成整份方案覆盖当前基线。
- 对非 Combo 请求手动删除 `stream_options`（应由 gate.js 自动设 `stream=true`）。
...
```
```
`hf-space/gate/package.json` 是必需的工程边界文件，用于将 `gate.js` 隔离在独立 CommonJS package 作用域内，避免被 OmniRoute `/app/package.json` 的 ESM 设置影响，同时显式声明 `express` 与 `http-proxy-middleware` 依赖。
